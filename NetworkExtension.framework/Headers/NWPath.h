 ...
 */
@property (nonatomic, readonly) NWPathStatus status API_AVAILABLE(macos(10.11), ios(9.0)) API_UNAVAILABLE(watchos) __WATCHOS_PROHIBITED;

/*!
 * @property expensive
 * @discussion Returns YES if the path is considered expensive, as when using a cellular data plan.
 */
@property (nonatomic, readonly, getter=isExpensive) BOOL expensive API_AVAILABLE(macos(10.11), ios(9.0)) API_UNAVAILABLE(watchos) __WATCHOS_PROHIBITED;
 ...
 */
- (BOOL)isEqualToPath:(NWPath *)path API_AVAILABLE(macos(10.11), ios(9.0)) API_UNAVAILABLE(watchos) __WATCHOS_PROHIBITED;
