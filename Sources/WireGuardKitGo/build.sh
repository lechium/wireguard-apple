...
GOOS=darwin GOARCH=arm64 CC=$PWD/clangwrap.sh CXX=$PWD/clangwrap.sh CGO_CFLAGS="-isysroot $SDK_PATH -arch arm64 -mtvos-version-min=12.0 -I$SDK_PATH/usr/include" CGO_LDFLAGS="-isysroot $SDK_PATH -arch arm64 -L$SDK_PATH/usr/lib/" CGO_ENABLED=1 go build -ldflags=-w -trimpath -pkgdir=$GOPATH/pkg/gomobile/pkg_darwin_arm64 -tags "tag1 ios" -v -o="$DESTDIR/libwg-go.a" -buildmode c-archive
